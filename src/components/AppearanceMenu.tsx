...
import { Palette, X, Sparkles, Check, Trash2, Loader2, Eye, EyeOff, Wand2 } from "lucide-react";
import { PRESETS, setThemePreset, setCustomTheme, getThemeId, getFxOff, setFxOff, applyTokens, type ThemeTokens } from "@/lib/appearance";
import { supabase } from "@/integrations/supabase/client";
import { useAuth } from "@/hooks/useAuth";
import { toast } from "sonner";

type SavedTheme = { id: string; name: string; tokens: ThemeTokens };
...
function AppearanceDialog({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    supabase.from("user_themes").select("id, name, tokens").order("created_at", { ascending: false }).then(({ data }) => {
      setSaved(((data ?? []) as unknown as SavedTheme[]));
    });
  }, [user]);

  const pickPreset = (id: string) => { setThemePreset(id); setThemeIdState(id); };
  const pickSaved = (t: SavedTheme) => { setCustomTheme(t.tokens); setThemeIdState("custom"); };
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/70 backdrop-blur-xl animate-pop-in"
      ...
      <div onClick={e => e.stopPropagation()}
        ...
        <div className="overflow-y-auto px-4 py-3 space-y-4 scrollbar-thin">
          ...
          {/* Presets */}
          <div>
            <h3 className="text-xs font-bold text-primary mb-2">تصاميم جاهزة</h3>
                ...
                return (
                  <button key={p.id} onClick={() => pickPreset(p.id)}
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      <span className="h-6 w-6 rounded-full border border-white/20" style={{ background: p.tokens.primary }}/>
                      <span className="h-6 w-6 rounded-full border border-white/20" style={{ background: p.tokens.accent }}/>
                      <span className="h-6 w-6 rounded-full border border-white/20" style={{ background: p.tokens.background }}/>
                ...
                {saved.map(t => (
                  <div key={t.id} className="flex items-center justify-between gap-2 p-2.5 rounded-2xl border border-white/10">
                    <button onClick={() => pickSaved(t)} className="flex-1 flex items-center gap-2 text-right hover:opacity-80">
                      <span className="h-6 w-6 rounded-full border border-white/20" style={{ background: t.tokens.primary }}/>
                      <span className="h-6 w-6 rounded-full border border-white/20" style={{ background: t.tokens.accent }}/>
